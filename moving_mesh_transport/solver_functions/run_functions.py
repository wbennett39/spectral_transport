...
class run:
    ...
    def random_IC(self, uncollided = True, moving = True, All = False):
        ...
        # plt.show()
    def custom_source(self, randomstart = False, sol_coeffs = np.array([[0.0], [0.0]]), phi_coeffs =np.array([[0.0], [0.0], [0.0]]),  uncollided = True, moving = True, All = False, input_phi_coeffs = None, input_A = None, input_coeffs = None):
        plt.ion()
        # plt.figure(1)
        ...
        solver = main_class(source_name, self.parameters, self.mesh_parameters)
        solver.precon_mat = None
        if randomstart == True:
            phi_coeffs = np.random.random(((solver.N_angles[0]+1) * solver.N_groups, solver.N_spaces[0], solver.Ms[0]+1))
            sol_coeffs = np.random.random((solver.N_spaces[0], solver.Ms[0]+1))
            input_source_coeffs =  np.random.random(((solver.N_angles[0]+1) * solver.N_groups, solver.N_spaces[0], solver.Ms[0]+1))
            if input_phi_coeffs is not None:
                phi_coeffs = input_phi_coeffs
            solver.fixed_source_coeffs = sol_coeffs
            solver.phi_coeffs = phi_coeffs
            solver.randomstart = True
            solver.input_source_coeffs = input_source_coeffs
            ...
            solver.load_custom_source(sol_coeffs, phi_coeffs, randomstart, input_A, input_coeffs) 
