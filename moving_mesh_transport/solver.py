        ...
        for nr in range(self.N_runs):
            for count, N_ang in enumerate(self.N_angles):
                ...
                xs, phi, psi, exit_dist, exit_phi, e, time, sol_matrix, angles, ws, edges, wavespeed_array, tpnts, left_edges, right_edges, wave_tpnts, wave_xpnts, T_front_location, mus, sol_object, uncollided_ob =  solve(
                    ...
                    self.fixed_source_coeffs, self.randomstart, self.chi, self.nu, self.sigma_f, self.legendre_moments, self.angular_derivative, self.Euler_dt_spacing, self.Euler_dt_num)
