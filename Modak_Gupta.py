...
def results(theta = 0.55, run_results = False, skip = 3, iterate_theta = False, sparse_time_points = 10):
    if run_results == True:
        ...
        for sigma_name in problem_list:
            ...
            run.load('modak_gupta', 'mesh_parameters_modak_gupta')
            run.mesh_parameters['sigma_func']['modak_gupta0'] = False
            run.mesh_parameters['sigma_func'][sigma_name] = True
    ...
    for iterator in range(5):
            ...
            eigen_vals = DMD_func3(Y_minus, ts,  integrator, sigma_t, skip = skip, theta = theta, sparse_time_points=sparse_time_points, try_WDMD=True)
