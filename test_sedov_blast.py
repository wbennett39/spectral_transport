...
for it in range(times.size):
    phi_bench = TS_bench(times[it], xs, g_interp, v_interp, sedov, beta = beta, transform=True, x0 = run.x0, t0 = run.t0, sigma_t = sigma_t, relativistic = True)
    RMSE_list[it] = RMSE(phi[it], phi_bench)

plt.loglog(times, RMSE_list, '-o')
plt.show()
